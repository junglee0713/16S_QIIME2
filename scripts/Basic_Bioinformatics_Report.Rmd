...
pander(s_Table, caption = "Samples with sufficient read counts by study group")
```
...
res <- getPrevalentTaxa(order_counts, least_prevalence = 0.001, top_taxa_n = 10)
...
```{r}
ReadCounts <- s %>% select(SampleID, demux_Read_Counts, denoise_Read_Counts, QC_Read_Counts) %>%
  arrange(desc(QC_Read_Counts))
datatable(ReadCounts, fillContainer = FALSE, options = list(pageLength = 7))
```
